...
There is not a experimental unit, as there is no experiment. One can also not say, what the experimental unit would be, if there is an experiment, as it depends on the setup of the experiment. As the study is observational, there is no real experimental design. The data collection was done at the authors' hospital itself, like described in section~\ref{datacollection}. As it is a observational study the effect of confounding variables was not tackeld. However one can make inferences about the relationship between independent variables and dependent variables.\par
Cofounding variables were not explicitly stated by the authors, ... %age could be one, but unsure
As the study was observational there was no intended control of any variable or factor, so the possible cofounding factors were not controlled either.\par
There can be a selection bias as the whole data is from a single center.\par
The clinical data was mesaured by the hospital as some other data (age, ...) were reported by the patiences.\par
The study itself is exploratory as in the study actually wanted to check if there might be a connection but did not already start with a quite clear theory, which should now be proved. For that reason a hypthesis is stated in the end however not already in the beginning. The objective was to explore risk factors associated with CLMN in PTC patients.
