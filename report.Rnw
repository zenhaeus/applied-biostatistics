...
The boxplot in of the data Figure~\ref{fig:boxplot} already suggested a difference between the two treatment groups, which can also be seen in Table~\ref{tab:means} for means and in Table~\ref{tab:variances} for variance.
