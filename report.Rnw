...
The boxplot ~\ref{fig:boxplot} of the data already suggested a difference between the two treatment groups, which can also be seen in the two tables for mean~\ref{tab:means} and for variance~\ref{tab:variance}.\\
The linear model solidifies our assumption. The anova and the TukeyHSD then show, that there is a siginicant difference between the two treatments.
