...
In the boxplot, which is combined with a dotplot, shown in Figure~\ref{fig:boxplot}, one can see that there might be a difference between the two treatments. However as the data from treamtment 1 is scattered, it is difficult to conclude anything, especially as there are not so many datapoints. Furtehrmore it is not possible to determine whether either or even both treatments are significantly different to the control group.
...
As a first step in our analysis we use the \texttt{lm} function to model the data. The model shows some evidence that there is a difference in the average growth, when treatment 2 is in use.
<<lm_summary_table, fig.height=4, warning=FALSE, echo=FALSE, results="asis">>=
plant.fit = lm(weight ~ group, data = plant.data)
print(xtable(summary(plant.fit)))
@
...
We use the \texttt{confint} function to calculate confidence intervals for the groups, the result is listed in Table~\ref{tab:confint}. However as the confidence intervals are overlapping, one can not conclude a siginficant difference from it.
...
The boxplot in of the data Figure~\ref{fig:boxplot} shows that there is some difference between the two treatment groups, which can also be seen in Table~\ref{tab:means} for means and in Table~\ref{tab:variances} for variance. However there are not so many data points and the data points from treatment 1 are quite scattered, which both leads to some uncertanty. %Bin mit dem satz, also dem letzten, noch nicht zufrieden
The linear model gives some further support for this assumption. The \texttt{anova} with a 5\%-confidence-level and the \texttt{TukeyHSD} with a 95\%-confidence-level then show, that there is a siginicant difference between the two treatments.
