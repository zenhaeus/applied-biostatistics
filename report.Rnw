...
As first analyzing step we use the \texttt{lm} function to model the data. The model has evidence that there is a difference in the average growth when treated with the second one.
