...
In this report we will carry out a exploratory analysis on the \texttt{PlantGrowth} dataset, which is included in the 'datasets' package of the base R~\cite{R} installation. The dataset contains the results from an experiment in which plant yields (measured by dried weight) were compared under a control and two different treatment conditions. The data consists of 30 observations of two variables (,weight', ,group').
...
The combined boxplot shown in Figure~\ref{fig:boxplot} of the data suggests that there is a difference of the 
...
print(xtable(tidy(plant.mean), caption = "Mean by group", label = "tab:means"))
print(xtable(tidy(plant.var), caption = "Variance by group", label = "tab:variances"))
@
\newpage
\section{Data Modeling}
As a first step in our analysis we use the \texttt{lm} function to model the data. The model shows evidence that there is a difference in the average growth when treated with the second one.
...
For further analysis we use \texttt{anova} on the fitted model. The result of the \texttt{anova} confirms that there is a difference between the groups.\\
The \texttt{confint} function is used to calculate confidence intervals for the groups.\\
The \texttt{TukeyHSD} shows that the difference is between the two treatments.
